...
export class LinkifyPipe implements PipeTransform {
  transform(value: string): any {
    return value.replace(/(?:(https?\:\/\/[^\s]+))/m, '<a href="$1">$1</a>'); // replace url
  }
}
...
@Component({
  ...
  template: `
    ...
    <div
      class="output"
      [innerHTML]="review_content | striphtml | linkify"
    ></div>
  `,
  styles: [
    `
...
@NgModule({
  ...
  declarations: [ReviewComponent, StripHtmlPipe, LinkifyPipe],
})
